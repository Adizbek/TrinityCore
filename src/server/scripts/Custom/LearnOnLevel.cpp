...
public:
    ...
    LearnSpellsOnLevelUp() : PlayerScript("LearnSpellsOnLevelUp") {
        uint64 temp[] = {
                ...
                20186, 20185, 20184, 20187, 25899, 24406, 50581, 30708, 31801
        };
    ...
    void LearnSpellsForNewLevel(Player *player, uint8 level) {
        ...
        for (uint32 i = 0; i < sSpellMgr->GetSpellInfoStoreSize(); ++i) {
            ...
            if ((spellInfo->AttributesEx7 & SPELL_ATTR7_ALLIANCE_ONLY && player->GetTeam() != ALLIANCE) ||
                (spellInfo->AttributesEx7 & SPELL_ATTR7_HORDE_ONLY && player->GetTeam() != HORDE))
                continue;
            ...
            for (SkillLineAbilityMap::const_iterator itr = bounds.first; itr != bounds.second; ++itr) {
                if (itr->second->spellId == spellInfo->Id && itr->second->racemask == 0 &&
                    itr->second->AutolearnType == 0) {
                    valid = true;
